...
(() => {
    ...
    [{
        ...
        url: `${(r=function(){let r;const s=document.getElementsByTagName("script");for(let d=0;d<s.length;d++){const a=s[d].getAttribute("src");if(!a)continue;if(s[d].hasAttribute("data-gorgias-loader-convert")&&(r=a.split("/").slice(0,3).join("/"),a.includes(`${o}=`))){if(window.CONVERT_SHOP_NAME)break;const e=new URLSearchParams(a.split("?")[1]).get(o);if(e){window.CONVERT_SHOP_NAME=e;break}}const c=s[d].getAttribute("id");if(a.includes(`
        $ {
            i
        } = `)||a.includes(`
        $ {
            t
        } = `)||[e,n].includes(c)){if(r=a.split("/").slice(0,3).join("/"),window.REVENUE_ADDON_ID)break;const e=new URLSearchParams(a.split("?")[1]);[i,t].forEach((n=>{const o=e.get(n);o&&(window.REVENUE_ADDON_ID=o)}))}}return{baseUrl:window.REVENUE_ADDON_CDN||r||"https://files.9gtb.com",revision:window.REVENUE_ADDON_REVISION||"e4c4b80"}}()).baseUrl}/script.js?rev=${r.revision}`
    }].map((e => {
