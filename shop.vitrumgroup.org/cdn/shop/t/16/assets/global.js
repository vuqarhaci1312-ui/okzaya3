...
class MenuDrawer extends HTMLElement {
    ...
    bindEvents() {
        ...
        this.querySelectorAll(
            'button:not(.localization-selector):not(.country-selector__close-button):not(.country-filter__reset-button):not([data-drawer-search-submit])'
...
class ModalDialog extends HTMLElement {
    constructor() {
        super();
        this.querySelectorAll('[id^="ModalClose-"]').forEach((button) => button.addEventListener('click', this.hide.bind(this, false)));
...
class SlideshowComponent extends SliderComponent {
    ...
    setAutoPlay() {
        ...
        this.sliderAutoplayButton = this.querySelector('.slideshow__autoplay, .reviews-slider__autoplay');
    ...
    togglePlayButtonState(pauseAutoplay) {
        if (pauseAutoplay) {
            this.sliderAutoplayButton.classList.add('slideshow__autoplay--paused', 'reviews-slider__autoplay--paused');
            this.sliderAutoplayButton.setAttribute('aria-label', window.accessibilityStrings.playSlideshow);
        } else {
            this.sliderAutoplayButton.classList.remove('slideshow__autoplay--paused', 'reviews-slider__autoplay--paused');
...
class ProductRecommendations extends HTMLElement {
    ...
    initializeRecommendations(productId) {
        ...
        this.observer = new IntersectionObserver(
            ...
            // Generous two-way margin: with two stacked recommendation rows mid-page,
            // the stock 400px bottom-only margin let fast scrolling get past the rows
            // before their content arrived — the late-loading rows then shifted the
            // page under the reader (and a downward-only margin never loads rows the
            // reader jumped past).
            {
    ...
    loadRecommendations(productId) {
        fetch(`${this.dataset.url}&product_id=${productId}&section_id=${this.dataset.sectionId}`)
            ...
            .then((text) => {
                ...
                // An empty result (no recommendations for this product/intent) would
                // otherwise leave the section's padded shell as a blank band.
